...
- Job-description quality is authoritative only when both structural completeness and task-catalog skill consistency pass. Changing a task's required-skill assignment revalidates active versions that use that task; an affected version leaves Human Resources review and returns to `منتظر رفع نقص` until the Department Manager corrects and explicitly resubmits it. Approved and archived historical versions are preserved.
