...
export function UserPreferenceProvider({ children, userId }: PropsWithChildren<{ userId: number }>) {
  const { mode, setMode } = useAppearance();
  const [sidebarCollapsed, setSidebarCollapsed] = useState(false);
  const [saveFailed, setSaveFailed] = useState(false);
  const preferenceRef = useRef<UserPreference>({ appearanceMode: mode, palette: "navyTeal", sidebarCollapsed });
  const sidebarTimer = useRef<number | undefined>(undefined);
  const query = useQuery({ queryKey: ["preferences", userId], queryFn: preferencesApi.get });
  ...
  const persist = useCallback(async (next: UserPreference, previous: UserPreference) => {
    preferenceRef.current = next;
    setSaveFailed(false);
    try { preferenceRef.current = await mutation.mutateAsync(next); }
    catch {
  ...
  const updateAppearance = useCallback((appearanceMode: AppearanceMode) => {
  ...
  const updateSidebarCollapsed = useCallback((collapsed: boolean) => {
    ...
    sidebarTimer.current = window.setTimeout(() => void persist(next, previous), 400);
  }, [persist]);

  useEffect(() => () => window.clearTimeout(sidebarTimer.current), []);
  const value = useMemo(() => ({ appearanceMode: mode, sidebarCollapsed, updateAppearance, updateSidebarCollapsed, saveFailed }),
    [mode, saveFailed, sidebarCollapsed, updateAppearance, updateSidebarCollapsed]);
  return <Context value={value}>{children}</Context>;
}

export function useUserPreferences() {
  const value = useContext(Context);
  if (!value) throw new Error("useUserPreferences must be used within UserPreferenceProvider");
  return value;
}
