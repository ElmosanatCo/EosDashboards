...
const cache = createCache({ key: "eos-rtl", stylisPlugins: [prefixer, rtlPlugin] });
...
export function AppThemeProvider({
  ...
  const storageKey = userId ? `eos.appearance.user.${userId}` : "eos.appearance.anonymous";
  const prefersDark = useMediaQuery("(prefers-color-scheme: dark)");
  const [mode, setModeState] = useState<AppearanceMode>(() => readStoredMode(storageKey));
...
export function useAppearance() {
  ...
  if (!value) throw new Error("useAppearance must be used within AppThemeProvider");
  return value;
}
