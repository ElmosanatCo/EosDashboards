...
export const initialTabState: TabState = { tabs: [homeTab], activeKey: homeTab.key };
...
export function tabReducer(state: TabState, action: TabAction): TabState {
  switch (action.type) {
    case "open": {
      ...
      return existing ? { ...state, activeKey: existing.key } : {
        tabs: [...state.tabs, action.tab], activeKey: action.tab.key,
      };
    }
    case "activate":
    ...
    case "close": {
      ...
      if (!target?.closable || (target.dirty && !action.confirmed)) return state;
      const index = state.tabs.indexOf(target);
      const tabs = state.tabs.filter((tab) => tab.key !== action.key);
      const activeKey = state.activeKey === action.key
    ...
    case "closeOthers": {
      ...
      if (!action.confirmed && state.tabs.some((tab) => tab.key !== keep.key && tab.dirty)) return state;
      return { tabs: keep.key === homeTab.key ? [homeTab] : [homeTab, keep], activeKey: keep.key };
    }
    case "closeAll":
      return !action.confirmed && state.tabs.some((tab) => tab.dirty) ? state : initialTabState;
    case "markDirty":
      return { ...state, tabs: state.tabs.map((tab) => tab.key === action.key ? { ...tab, dirty: action.dirty } : tab) };
    case "setState":
      return { ...state, tabs: state.tabs.map((tab) => tab.key === action.key ? { ...tab, state: action.state } : tab) };
    case "clear":
...
export function restoreTabs(serialized: string | null): TabState {
  ...
  try {
    ...
    if (!Array.isArray(parsed.tabs) || !parsed.tabs.some((tab) => tab.key === homeTab.key) ||
        !parsed.tabs.some((tab) => tab.key === parsed.activeKey)) return initialTabState;
    return parsed;
  } catch { return initialTabState; }
}
