...
export function AppShell() {
  const { sidebarCollapsed, updateSidebarCollapsed, saveFailed } = useUserPreferences();
    ...
    <Box sx={{ height: "100dvh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <Box sx={{ display: "flex", flex: 1, minHeight: 0 }}>
        ...
        <Box sx={{ flex: 1, minWidth: 0, width: sidebarOpen ? `calc(100% - ${sidebarWidth}px)` : "100%", display: "flex", flexDirection: "column" }}>
          <WorkspaceTabs />
          {saveFailed ? <Alert severity="warning">ذخیره تنظیمات انجام نشد و مقدار قبلی بازگردانده شد.</Alert> : null}
