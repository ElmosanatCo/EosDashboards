...
- Linked Google sign-in is being implemented in an isolated worktree. The first three independently verified slices add the `ExternalIdentityLinks` persistence model, unique Google email/subject constraints, optimistic-concurrency row version, EF migration `20260903103049_ExternalIdentityLinks`, and tracked lookup repository; the administrator provisioner now accepts an optional Google email only through its hidden-entry path. A blank entry leaves links unchanged; an explicit update preserves any already-bound Google subject. A verified pre-linked Google identity can now bind its stable subject and receive the normal eight-hour application session; unknown, unverified, and inactive identities are denied without OTP or user creation. The existing noninteractive deployment bridge supplies a blank optional field for compatibility. No personal identity value is printed or documented; five domain, 15 focused provisioning-application, 13 focused Google/session-application, and 17 focused SQL-backed provisioning tests passed against the safe integration-test catalog.
