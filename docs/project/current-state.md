...
- Linked Google sign-in is being implemented in an isolated worktree. The first independently verified slice adds the `ExternalIdentityLinks` persistence model, unique Google email/subject constraints, optimistic-concurrency row version, EF migration `20260903103049_ExternalIdentityLinks`, and tracked lookup repository. It contains no personal identity data; four domain tests and 21 focused SQL-backed mapping/repository/constraint tests passed against the safe integration-test catalog.
