...
- At the start of every task, read `current-state.md`, `README.md`, and this `standards.md` before reading task-specific documents or taking task action. Then load only the canonical documents relevant to the task.
...
- Treat material failures as learning opportunities, not as reasons for blind repetition. For every material tooling, environment, deployment, test, or integration failure, record the symptom, root cause, durable remedy, safe verification evidence, and prevention rule in the appropriate canonical source before ending the task.
- When a running development API holds build output assemblies, stop that API before running a backend test or build that targets the same projects; restart it only after the focused verification completes. This avoids false test/build failures caused by locked DLLs and keeps the browser smoke environment available at the final checkpoint.
- Record reusable development and environment remedies in this document so future work can avoid repeating the same failed operation. Record task-specific current state in `current-state.md`, and use a decision record when the rationale is consequential. Do not turn these documents into raw incident logs.
...
- Use a compact, desktop-first operational workspace with responsive mobile behavior. Do not use decorative gradients, glassmorphism, floating panels, oversized rounded cards, excessive whitespace, generic hero sections, or consumer-SaaS visual patterns; only the approved restrained theme-accent gradients in the fixed shell and active workspace tab are allowed.
...
- Offer teal, indigo, emerald, amber, orange, and rose accent choices. Accent colors affect interaction highlights only; semantic status colors remain fixed: green for approved/healthy, amber for pending or attention, red for rejected/critical, and blue or teal for informational or active states. Maintain accessible contrast in every appearance and palette.
- Style all native scrollbars from the active appearance and accent palette: use the themed surface for the track, the palette accent for the thumb, and a stronger accent on hover. Apply the rule globally to the shell, workspace pages, panels, tables, and independently scrolling regions; do not introduce page-specific scrollbar colors.
- Persist appearance, palette, sidebar state, and the shell-gradient preference per user in the database; cache the last applied appearance and palette locally so the sign-in page renders with the last selected theme without a flash. The gradient preference defaults to enabled, and disabling it removes active-tab and workspace-content gradients while leaving the main header solid and retaining the active-sidebar-menu gradient.
...
- Present opened SPA pages as closable internal workspace tabs. Keep the first tab, `خانه`, fixed and non-closable. Pages opened from the menu, a table row, an AI insight, a report, or a dashboard action open or activate a workspace tab; reopen an exact page by activating its existing tab. Use concise Persian tab titles, icons, active states, close controls, and an unsaved-change marker when required. Use a compact, quiet inactive state and an amber/gold underline for the active tab; the active tab may also use a restrained theme-accent gradient that starts at its lower edge and fades upward before the midpoint when the user's shell-gradient preference is enabled. The active page workspace may use a very subtle upper-right-to-lower-left theme gradient that fades within the first quarter under the same preference; cards and tables remain flat. On narrow screens, make the strip horizontally scrollable and provide an accessible overflow menu for open tabs.
- Render only the active tab's page tree while preserving approved serializable state for inactive tabs.
- Include brand, current role and organizational scope, a compact global command search, theme control, notifications, and user profile in the header. The search returns only pages, permitted operations, and later eligible in-dashboard elements that the current user may access; choosing a result opens or activates its internal workspace tab. Do not expose unavailable targets through search. Keep the search input in the primary interface typeface at normal weight. The fixed header uses the active theme's primary color across its full height with contrast text and icons; the active sidebar item may use a low-contrast accent wash and a narrow physical-right indicator. The status bar shows data synchronization state, last-update time, active organization or department, system health, and the existing required application version and Persian-calendar local-time information; display its clock with hours and minutes only.
- Place a shared help icon in the fixed header beside the user menu. When selected, open a readable guide for the active workspace tab with the fixed sections `وظایف این صفحه`, `امکانات`, `شیوه انجام کار`, and `محدودیت‌ها`; keep the content truthful to the approved stage and update it when the related capability changes.
...
- Require an explicit Persian confirmation before every user-visible delete, removal, archive, deactivation, or reactivation action. The confirmation must identify the target and action; cancel, close, and backdrop dismissal must leave state unchanged and must not send a mutation request.
- Every form mutation must render an actionable Persian success or error state; a rejected request must never fail silently and must preserve the user's entered values. Duplicate values must be translated to a stable, field-appropriate conflict message. Soft-deactivated catalog items must remain discoverable through an inactive/all filter and have an authorized reactivation action.
...
- When invoking the bundled Python runtime from Windows for Unicode inspection or validation, set `PYTHONIOENCODING=utf-8` (or use an equivalent explicit UTF-8 output configuration) before emitting non-ASCII paths or values. A default `cp1252` console is not an acceptable fallback.
...
- Keep agent token use cost-conscious: load only the minimum startup context plus task-relevant documents, avoid redundant scans and repeated diagnostics, and do not rerun a successful command unless code, configuration, environment, or the claim being verified changed.
- Do not repeat a failed test, build, deployment, or diagnostic operation unchanged. Before retrying, identify the likely cause and change or verify the relevant code, configuration, environment, input, or diagnostic hypothesis. If the failure is environmental, record the durable workaround or fix in this document after safe verification.
- On Windows, do not run frontend test/build commands concurrently when both invoke `sync:resources`; the shared `frontend/public/generated-assets` cleanup can fail with `EPERM`. Run those commands serially.
- A fresh Git worktree may not contain restored .NET assets files. If a solution build reports `NETSDK1004` for a missing `obj/project.assets.json`, run `dotnet restore` once in that worktree, then rerun the build with `--no-restore`; do not treat the initial missing-assets failure as a source regression.
- Run frontend npm and Prettier checks with `frontend` as the working directory, or use an explicit `npm --prefix frontend ...` form from the repository root; first verify the selected directory contains `package.json`. For IIS builds, run `npm run typecheck` and `npm run build:iis` from `frontend`, and never retry a root-level npm command after its `package.json` error.
- Before any finalization or local IIS publication, use `scripts/Finalize-LocalIisRelease.ps1` from the repository root. It checks the current Windows token with `WindowsPrincipal.IsInRole(Administrator)`, requests UAC elevation with `Start-Process -Verb RunAs` when needed, builds both artifacts from the same committed source, invokes the guarded publisher, and verifies the HTTPS live/ready, UI entry, and SPA refresh probes. A canceled UAC request is a failed preflight, not a successful no-op; do not continue or infer that IIS changed. The guarded publisher itself must continue refusing non-administrator tokens.
- If PowerShell blocks the repository publisher because of Execution Policy, use a process-scoped bypass only: `Set-ExecutionPolicy -Scope Process -ExecutionPolicy Bypass -Force`, then invoke the script. Never change the machine or user policy for this release. Prefer one physical command line when copying deployment commands to avoid Markdown backslash/ backtick corruption; pass migration identifiers with their plain underscore.
- When using a temporary Playwright specification outside the repository, keep it under a dedicated temporary directory and set the temporary config's `testDir` to that directory; scanning the shared Windows temp root can encounter unrelated locked folders and fail with `EPERM` before tests start.
...
- When running EF design-time commands with `--no-build`, build the complete solution first so the startup project and its referenced Infrastructure assembly are current. Otherwise an old API assembly can produce false model warnings or an incorrect migration; verify the generated migration contains no unexpected shadow properties before proceeding.
- If a migration is regenerated after a failed or partial local attempt, first reconcile the EF snapshot through the EF migration workflow, then rebuild the complete solution and generate the intended migration again. Do not hand-edit a snapshot to hide a pending model change; verify the final migration against a disposable/integration database before broader tests.
- Excel job-description adapters must use explicit UTF-8 XML/package handling, tolerate optional omissions and explanatory rows, preserve useful extra columns in the free-text description, and keep unmatched typed catalog values as review suggestions rather than silently creating catalog entries.
...
- On 2026-09-05 the repository-wide frontend Prettier check reported existing formatting differences in `src/pages/DepartmentManagementPage.test.tsx` and `src/pages/UserManagementPage.test.tsx`; the current UI release changed neither file. The other release checks passed, so the safe remedy is to leave this unrelated baseline unchanged and use focused formatting checks for release changes until it is intentionally normalized.
