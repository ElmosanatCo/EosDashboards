...
- Run frontend npm and Prettier checks with `frontend` as the working directory, and use the exact project filenames under `backend/tests` for focused .NET test commands; this avoids false verification failures caused only by repository-root paths or abbreviated project names.
