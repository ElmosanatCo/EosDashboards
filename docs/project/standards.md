...
- Before any finalization or local IIS publication, check the current Windows token with `WindowsPrincipal.IsInRole(Administrator)`. If the session is not elevated, stop before building release artifacts or invoking the publisher and ask for an elevated PowerShell session. A canceled UAC request is a failed preflight, not a successful no-op; do not continue or infer that IIS changed. Once elevated, run the publisher from the repository root with absolute or repository-root-resolved API/UI artifact paths, then verify the HTTPS live/ready, UI entry, and SPA refresh probes.
...
- On 2026-09-05 the repository-wide frontend Prettier check reported existing formatting differences in `src/pages/DepartmentJobDescriptionsPage.test.tsx`, `src/pages/DepartmentManagementPage.test.tsx`, `src/pages/home/homeContent.ts`, and `src/pages/UserManagementPage.test.tsx`; the current UI release changed none of those files. Keep the full-check finding visible and use focused formatting checks for release changes until that baseline is intentionally normalized.
